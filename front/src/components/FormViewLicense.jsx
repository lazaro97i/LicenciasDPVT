...
const FormViewLicense = ({ licenseFound }) => {
  ...
  const sendFile = async () => {
    setLoading(true)
    if (inpFile.current.value === "" || inpFile.current.value === null) {
      toast.error("Debe ingresar un legajo")
      setLoading(false)
    } else {
  ...
  return (
    <>
      ...
      <form onKeyDown={(e) => { e.code === 'Enter' ? sendFile() : null }} className="border rounded-sm w-full max-w-[600px] min-h-[185px] flex flex-wrap justify-center gap-x-2 mt-20 py-10">
        {loading
          ? <Loader />
          : <>
